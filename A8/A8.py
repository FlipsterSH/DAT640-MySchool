...
def load_test_features(filepath: str) -> Dict[str, Dict[str, List[float]]]:
    ...
    Returns:
        Dictionary with query IDs as keys and a dictionary of document IDs and
        corresponding query-document feature values as values.
    """
...
def load_training_data(filepath: str) -> Tuple[List[List[float]], List[int]]:
    ...
    with open(filepath, 'r') as csvfile:
        ...
        for row in reader:
            # Assuming the label is the last element and features are the preceding elements
            *feature_values, label = row
            features.append(ast.literal_eval(feature_values))
